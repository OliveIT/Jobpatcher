            ...
            <div className="gx-customer-profile-tab-panel-content">
                <div className="gx-flex-row gx-w-100 gx-justify-content-between gx-align-items-center" style={{height: '58px'}}>                
                    <div className="gx-fs-md gx-font-weight-semi-bold gx-d-none gx-d-lg-block">                
                        <div className="gx-flex-row  gx-align-items-center">    
                            <IntlMessages id="customer.job.panel.invoices.subt" values={{ value: data.length }} />
                        </div>                    
                    </div>
                    <div className="gx-flex-row">                 
                        <span className="gx-d-none gx-d-lg-block">
                            <SearchBox styleName="gx-lt-icon-search-bar-lg gx-customer-search"
                                placeholder={formatMessage({id: 'customer.job.panel.invoices.searchjobs'})}
                ...
                <Widget styleName="gx-card-full gx-customer-widget gx-customer-panel-table-widget">
                    <table className="gx-profile-table gx-customer-panel-table">
                        <thead>
                            <tr>
                                <th><IntlMessages id="customer.job.panel.invoices.th.jobid"/></th>
                                <th><IntlMessages id="customer.job.panel.invoices.th.customer"/></th>
                                <th><IntlMessages id="customer.job.panel.invoices.th.issuedate"/></th>
                                <th><IntlMessages id="customer.job.panel.invoices.th.duedate"/></th>
                                <th><IntlMessages id="customer.job.panel.invoices.th.amount"/></th>
                                <th><IntlMessages id="customer.job.panel.invoices.th.status"/></th>
                                <th></th>
                            </tr>
                        </thead>
                        <tbody>
                            { 
                                data.map( (item, index) => 
                                    <tr>
                                        ...
                                        <td>
                                            <Select className="gx-customer-profile-panel-combobox" defaultValue={item.status} style={{width: '180px'}}
                                                    suffixIcon={<i className="gx-combo-icon material-icons">expand_more</i>}>
                                                {
                                                    invoice_status.map((status, idx) =>
                                                        <Option value={idx}>
